...
@values = ();
eval q{
	use Lexical::Sub::Patched foo => sub () { rand() < 2 ? 123 : 0 };
	push @values, foo;
};
is $@, "";
...
@values = ();
eval q{
	use Lexical::Sub::Patched foo => sub () { 123 };
	push @values, foo;
};
is $@, "";
is_deeply \@values, [ 123 ];
...
@values = ();
eval q{
	use Lexical::Sub::Patched foo => sub ($) { $_[0]+1 };
	push @values, foo;
};
isnt $@, "";
is_deeply \@values, [];
...
@values = ();
eval q{
	use Lexical::Sub::Patched foo => sub (@) { "a", map { $_+1 } @_ };
	push @values, foo;
};
is $@, "";
is_deeply \@values, [ "a" ];
...
@values = ();
eval q{
	use Lexical::Sub::Patched foo => sub { "b", map { $_+1 } @_ };
	push @values, foo;
};
is $@, "";
is_deeply \@values, [ "b" ];
...
@values = ();
eval q{
	use Lexical::Sub::Patched foo => sub (&) { "c", $_[0]->()+1 };
	push @values, foo;
};
isnt $@, "";
is_deeply \@values, [];
